...
def convert_to_voc(root_path='',image_path=''):
    """
    将充电宝数据集转换为voc格式
    :return:
    """
    ...
    img_sizes = []
    # 读取所有label
    for label_file_path in sorted(os.listdir(osp.join(root, ''))):
        print(label_file_path)
        labels = pd.read_csv(osp.join(root, '', label_file_path), delimiter=' ', header=None).values
        objects = ''
        for label in labels:
            # 只检测两类目标
        ...
        img_size = Image.open(osp.join(image_path, img_name)).size
        # 拼接label xml
        ...
        if not os.path.exists('./portable/VOCdevkit/VOC2012/Annotations'):
            os.makedirs('./portable/VOCdevkit/VOC2012/Annotations')
